...
(
	CControl,
	{
	    ...
	    _paint: function() {
	        ...
	        $(this._sInstance + '_place').style.fontSize = getFontSize(this._sInstance + '_place', 39);
	        this._validate();
	    },
	...
	CFieldBlock,
	{
	    ...
	    _paint: function() {
	        ...
	        if (this._aChunks.length) {
	            ...
	            if (this._sType != "SINGLE_STATIC") {
	                sContent +=
	                    '<td align="center" style="width: 22px; height: 60px; background: url(\'./img/ui/field/ld.gif\');"></td>';
	            }
	            for (var i = 0; i < this._aChunks.length; i++) {
	                if (this._sType == "SINGLE_FRAMED" || this._sType == "SINGLE_STATIC") {
	                    sContent +=
	                    ...
	                    if (this._aChunks[i].type == "STATIC") {
	                        sContent +=
	                        '<td class="field_chunk" id="' + this._sInstance + '_chunk_' + i + '_place" align="center" style="width: 1144px; height: 60px; background: url(\'./img/ui/field/cd.gif\') repeat-x;"></td>';
	                        this._aChunks[i]["place"] = this._sInstance + "_chunk_" + i + "_place";
	                    }
	                    else if (this._aChunks[i].type == "DYNAMIC") {
	                        sContent +=
	            ...
	            if (this._sType != "SINGLE_STATIC") {
	                sContent +=
	                    '<td style="width: 22px; height: 60px; background: url(\'./img/ui/field/rd.gif\');"></td>';
	            }
	            sContent +=
	                '  </tr>' +
                    '</table>';
	...
	CControl,
	{
	    ...
	    processValue: function(sValue) {
	        switch (sValue) {
	            case "BACKSPACE":
	                if (this.isEmpty()) {
	                    this._nCurrentBlock = this.getOneEmpty();
	                    this._aBlocks[this._nCurrentBlock].setActive(true);
	                }
	                else {
	    ...
	    _paint: function() {
	        ...
	        if (this._aBlockCfgs.length) {
	            ...
	            sContent +=
                    ...
                    '</table>' +
                    '</div>';
	            sContent += this._sFooter.toFormat("color: #FF4800; width: " + this._sFieldWidth + "px;");
	            insertContent(this.getPlaceId(), sContent);
	            for (var i = 0; i < this._aBlockCfgs.length; i++) {
