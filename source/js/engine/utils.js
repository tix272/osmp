...
var nNoCommission = promotion();
var sPromotion;
var top8info = [[], [], []];
var oMaratlData = {};
var bComissInfo = true;
var bUnekt = getUnekt();
var bCom = false;

function getUnekt() {
    var bREsult = false;
    if (typeof indexConfig !== "undefined" && indexConfig !== null && typeof indexConfig["hostData"] !== "undefined" && typeof indexConfig["hostData"]["unekt"] !== "undefined" && indexConfig["hostData"]["unekt"].toString() == "1") {
        bREsult = true;
    }
    return bREsult;
}
...
function hComission() {
    if (typeof indexConfig !== "undefined" && indexConfig !== null && typeof indexConfig["hostData"] !== "undefined" && indexConfig["hostData"]["hidecommission"] == "1") {
        bComissInfo = false;
    }
}
...
function promotion() {
    var nResult = 0;
    if (typeof indexConfig !== "undefined" && indexConfig !== null && typeof indexConfig["hostData"] !== "undefined" && typeof indexConfig["hostData"]["promotion"] == "string") {
        sPromotion = indexConfig["hostData"]["promotion"];
    }
    switch (sPromotion) {
        case "0":
            nResult = 5;
...
function checkUsageStep()
{
	...
	if (diff > 180) document.location.href = './index.html';
};
...
function clearShadov(clearName)
{
    var a = document.body.childNodes,
        nLength = a.length - 1;
    while (a.length) {
        if (a[nLength].name == clearName) {
            destroy(a[nLength]);
            nLength--;
        }
        else
...
function createElement(oParam) {
    if ($isNoU(oParam.name)) return false;
    var o = document.createElement(oParam.name);
    if (!$isNoU(oParam.id))
        o.id = oParam.id;
    if (!$isNoU(oParam.style)) {
        for (var f in oParam.style) {
            if (oParam.style[f]["_typeName"] == "String" && f !== "_typeName")
                o.style[f] = oParam.style[f];
        }
    }
...
function getFontSize(sObj, nMaxSize) {
    var oInput = $(sObj),
        oP = $("format_text_size"),
        nResultSize = nMaxSize,
        nWidth = parseInt(oInput.offsetWidth, 10),
        n;

    oP.style.fontSize = nMaxSize;
    oP.innerHTML = oInput.value;
    while (nWidth < parseInt(oP.offsetWidth, 10)) {
        oP.style.fontSize = nResultSize.toString() + "px";
        nResultSize--;
    }
    return nResultSize;
}

function getScriptFF(sFileName, sPath, oThis, func) {
    var js = document.createElement("script"),
        body = document.body;
    js.type = "text/javascript";
    js.src = sPath + sFileName + '.js';
    if (oThis == null) {
        js.onload = func;
    }
    else {
        js.onload = $delegate(oThis, func);
    }
    js.id = sFileName;
    body.appendChild(js);
    document.body.removeChild(js);
...
function getLogo(oElement) {
    if (!$isNoU(oElement) && !$isNoU(oElement["__objects"])) {
            ...
            if (oElement["__objects"][j]["__type"] == "logos" && !$isNoU(oElement["__objects"][j]["__objects"]))
                for (var x = 0; x < oElement["__objects"][j]["__objects"].length; x++)
                    if (oElement["__objects"][j]["__objects"][x]["type"] == "standard") {
                        return oElement["__objects"][0]["__objects"][0]["img"];
                    }
    }
}

function setData(sName, sValue) {
    var val = (typeof sValue == "undefined") ? "true" : sValue;
    try { Maratl.ProcessCommand(sName, val); } catch (e) { }
}
