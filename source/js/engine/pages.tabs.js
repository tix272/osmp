    ...
    CTab,
    {
        ...
        _prepareTemplate: function () {
            ...
            if (!$isNoU($(this._sPlace))) {
                for (var i = 0; i < this._aDispButton.length; i++) {
                    if (!$isNoU(this._aDispButton[i]["disp_name"]) || !$isNoU(this._aDispButton[i]["altName"])) {
                        sContent += ['<div id="', this._sInstance, "_e_", i, '" class="element-place">',
                	        '<div id="', this._sInstance, "_dispButton_", (i + 1), '"></div>', '</div>'].join('');
                    }
                }
        ...
        _createButton: function () {
            ...
            for (var i = 0; i < this._aDispButton.length; i++) {
                ...
                if (!$isNoU(this._aDispButton[i]["__objects"]))
                    aExtra = this._aDispButton[i]["__objects"];

                if (this._aDispButton[i].type == "disp_button")
                    sType = "disp_button";

                if (!$isNoU(this._aDispButton[i].disp_type))
                    sDispType = this._aDispButton[i].disp_type;
                ...
                if (!$isNoU(this._aDispButton[i].pageId))
                    sPageId = this._aDispButton[i].pageId;

                if (sType == "button")
                    sValue = this._aDispButton[i].altName;
                else
                    sValue = $(this._aDispButton[i].disp_name).value;

                oPlace = $(this._sInstance + "_dispButton_" + (i + 1));
                if (sValue.length) {
                    if (oPlace) {
                        oElement = new CDispButton(this, "el" + (i + 1), oPlace.id, sValue, sDispType, sDispDesc, sName, sPageId, aExtra);
                    ...
                    if (oPlace) {
                        oPlace.parentNode.style.display = "none";
                    }
                }
            }
        },
        ...
        _elementClick: function (sender, eargs) {
            for (var i = 0; i < eargs.extras.length; i++) {
                this._oParent._oPostData[eargs.extras[i].name] = eargs.extras[i].value;
            }
...
(
    CTab,
    {
        ctor: function (oParent, sInstance, sCtrlPlace, sId, aFields, sFieldType) {
        ...
        _renderFields: function (sPlace) {
            ...
            var bVisible = true;
            var k = 0;
            while (k < this._aFields.length) {
                if (!$isNoU(this._aFields[k]["disp_name"]) && $(this._aFields[k]["disp_name"]).value.length == 0) {
                    this._aFields[k]["visible"] = "false";
                }
                if (!$isNoU(this._aFields[k]["visible"]) && this._aFields[k]["visible"] == "false") {
                    this.addPostParam(this._aFields[k]);
                    this._aFields.splice(k, 1);
                }
                k++;
            }
            for (var i = 0; i < this._aFields.length; i++) {
                sContent +=
                    ...
                    '    <tr>' +
                    '     <td id="' + (this._sInstance + '_fld_' + this._aFields[i]["name"] + "_place") + '" valign="top"></td>';
                    '    </tr>';
                if (!$isNoU(this._aFields[i]["footer"]) && this._aFields[i]["footer"] != " ") {
                    sContent +=
            ...
            for (var i = 0; i < this._aFields.length; i++) {
                oFld = new CDIField(this, ('_fld_' + this._aFields[i]["name"]),
                                        (this._sInstance + '_fld_' + this._aFields[i]["name"] + "_place"),
                                        this._aFields[i], this._sFieldType);
    ...
    CTab,
    {
        ctor: function (oParent, sInstance, sCtrlPlace, sId, aCombined) {
            ...
            for (var i = 0; i < aCombined.length; i++) {
                if (aCombined[i].type == 'disp_button' || aCombined[i].type == 'button')
	    ...
	    _renderFields: function(sPlace) {
	        ...
	        for (var i = 0; i < this._aDispCombined.length; i++) {
	            if ((!$isNoU(this._aDispCombined[i]["disp_name"]) && $(this._aDispCombined[i].disp_name).value.length != 0) || 
	                (!$isNoU(this._aDispCombined[i]["altName"]) && this._aDispCombined[i].altName.length != 0)) {
                    if (this._aDispCombined[i].type == 'disp_button' || this._aDispCombined[i].type == 'button') {
                        sContent += 
                        '<table cellpadding="0" cellspacing="0" border="0">' +
                        '<tr>' +
                        '   <td align="left" valign="middle" width="301" height="121">' +
                        '       <div id="' + this._sInstance + '_e_' + nButton + '" class="element-place">' +
                	    '           <div id="' + this._sInstance + '_dispButton_' + (nButton + 1) + '"></div>' + 
                	    '       </div>' +
                        '   </td>';
                        if (this._aDispCombined[i + 1] == 'disp_input' && !$isNoU(this._aDispCombined[i + 1]["nobr"]) && this._aDispCombined[i + 1]["nobr"] == 'false')
                            sContent += '</tr></table>';
                        nButton++;
                    }
                    if (this._aDispCombined[i].type == 'disp_input') {
                        if (!$isNoU(this._aFields[nDisp]["nobr"]) && this._aFields[nDisp]["nobr"] == 'true')
	    ...
	    _createField: function () {
    	    ...
    	    for (var i = 0; i < this._aFields.length; i++) {
    	        ...
    	        if ($(this._aFields[i]["disp_name"]).value.length != 0) {
    	            if (!$isNoU(this._aFields[i]["nobr"]) && this._aFields[i]["nobr"] == "false")
    	                type = "short";
                    oFld = new CDIField(this, ('_fld_' + this._aFields[i]["name"]), (this._sInstance + '_fld_' + this._aFields[i]["name"] + "_place"), this._aFields[i], type);
                    oFld.attachListener("onValid", $delegate(this, this._onFieldValid));
                    this._aFldObjects.push(oFld);
                    oFld.render();
                    if (i == 0) {
...
(
	CTab,
	{
	    ...
	    _onFieldValid: function (sender, eargs) {
	        // log start
	        var sTemp = sender.getValue().replace(/</g, '(');
	        _log('<v v="' + sTemp + '"></v>');
	        // log stop
	    ...
	    _renderKeyboard: function (sPlace) {
	        if (!$isNoU($(sPlace))) {
	            ...
	            attachPropertyChangeListener("BarcodeData", $delegate(this, this._prekbKeyPressed));
	        }
	    },
	    ...
	    _prekbKeyPressed: function (oEventInfo) {
	        // log start
	        _log('<b v="' + oEventInfo.srcElement.value + '"></b>');
	        // log stop
	        var aString = oEventInfo.srcElement.value.split(''),
	            o = { "key": "" };
	        while (aString.length) {
	            o.key = aString.shift();
	            if (o.key !== "\\")
	                this._kbKeyPressed(null, o);
	        }
	    },
...
(
	CInputTab,
	{
	        ...
	        if (!$isNoU($(this._sPlace))) {
	            sContent +=
	                ...
	                '                <td align="center" valign="middle" class="provider-element">' +
	                			getProviderButton(this._sPrvName, getFileName(this._sLogo)) +
	                '                </td>' +
	                '              </tr>' +
	                '            </table>' +
	                '          </td>' +
	    ...
	    _renderBS: function (sPlace) {
	        var oBtn = new CImageButton(this, "bs_btn", sPlace, "BACKSPACE", "./img/ui/delbtn.gif");
	    ...
	    _prepareTemplate: function() {
	        ...
	        if (!$isNoU($(this._sPlace))) {
	            ...
	            sContent +=
	                ...
	                '            </table>' +
	                '          </td>' +
	                '          <td id="' + this._sInstance + '_fields_place" align="center" valign="top" style="width: 630px;"></td>' +
                    '          <td valign="middle"><div id="' + this._sInstance + '_bs" style="width: 301px; height: 121px;"></div></td>' +
	                '        </tr>' +
	                '      </table>' +
	                '    </td>' +
